...
The workflow intended to be used with \texttt{ASpli} is divided in four main 
steps (Figure \ref{fig:ASpliStructure}). For each of these steps
ASpli provides a single or few functions to use:
CHEQUEAR NOMBRES DE FUNCIONES. HAY QUE PONERLAS aca tambien
\begin{enumerate}
  \item INPUT DATA: Reading of genomic alignments: \texttt{ loadBAM() } and Feature extraction: \texttt{ binGenome() }
  \item Read counting: \texttt{ readCounts() }, \texttt{ binCounts() }, \texttt{ junctionsCounts() }
  \item Differential gene expression and bin usage: \texttt{ binDUreport(), \texttt{ junctionDUreport() } }
...
ASpli defines XXX four major objects classes used to store data and perform
operations:
CHEQUEAR
\begin{description}
  \item[ASpliFeatures] Contains genomic coordinates
  \item[GAlignments] Contains BAM alignments in format GRanges
...
\caption{ Workflow of ASpli pipeline and prerequisites.  
Each arrow represents one of the main steps of a typical analysis. Rounded boxes are objects created by ASpli functions.  Names under arrows are functions included in ASpli used to generate each kind of object. }
...
\subsubsection{Genomic alignment files loading: \texttt{loadBAM} method}
\label{secGAFL}
...
CHECK FUNCTION parameters and memory usage etc
...
\textbf{Chequear nombre de las funciones}

The following parameters should be specify:
\begin{itemize}
...
To analyze putative retention events for intron I, \texttt{ASpli} considers the 
...
single read overlaps more than one exon, it is counted only once. Note that \textbf{one read can
...
\subsubsection*{The binCounts and ASpliJCounts objects }
...
\subsection{Differential gene, bin and junction usage estimation. Differential signals. }
\subsubsection{ \texttt{binDUreport} o \texttt{gbDUreport}: A Coverage-based differential signals analyisis of alternative splicing. }
\label{sec:du}



%Bins and junctions are treated similarly for DU analysis. 
\texttt{ASpli} takes into account bins and
junctions from expressed genes (see section \secref{sec:de}) if they have more
supporting reads than a threshold (5 reads by default) in at least one 
condition. 

{\em External bins} do not participate in alternative splicing, therefore are usually
excluded from the analysis. However, as an {\em external bin} for one isoform can overlap
to a non external bin from other isoform that can participate in alternative
splicing regulation, ASpli allows to optionally include the {\em external bins} in
the analysis. In addition, {\em original intron} bins can be excluded from the analysis as an option. 
...
Read counts from bins/junctions can not be used naively for differential usage analysis. Counts needs to be adjusted
to remove the signal coming from changes in gene expression. To perform this, ASpli 
provides three alternatives: 



\begin{enumerate}
   \item{\bf Pseudo-counts}. In this case, original bin/junction counts are normalized to gene-level counts considering:

\begin{equation}
  \hat{b}_{ijk} = \frac{ b_{ijk}}{ g_{jk}/g_{j.}}
\end{equation}

\noindent where $\hat{b}_{ijk}$ and $b_{ijk}$ are the adjusted and raw bin-count for bin $i$ of gene $j$ in sample
$k$ respectively. $g_{jk}$ is the raw gene count for gene $j$ in sample $k$ and $g_{j.}$ is the mean raw count for gene $j$ across all samples. Integer rounded adjusted bin counts,$[\hat{b}_{ijk}]$ , are then analyzed using canonical edgeR functionality. 


   \item {\bf Gene expression offset}. Use \texttt{edgeR}'s generalized linear models considering  gene-expression offset terms. Offsets  reflect changes in gene expression levels and are used to correct the observed bin usage in the statistical analysis. The are several ways to estimate the offset vector:

\begin{itemize}
  \item Use gene counts (gene mode):
  \begin{enumerate}
     \item Use only raw counts.
     \item Fit a generalized linear model from gene counts.	
  \end{enumerate}
  \item Use the sum of exonic bins from each gene (bin mode). Notice that in this mode some reads might be
  counted more than once, because they might  overlap to more than one bin. This mode is not available for junctions.
\end{itemize}       


   \item {\bf diffSpliceDGE}. This alternative use differential bin usage estimation to \texttt{edgeR}'s 
         \texttt{ diffSpliceDGE }. This is available only for bins. This method uses an strategy
          similar to the offset methodology. ASpli allows its use with the method \texttt{DUreportBinSplice}.
\end{enumerate}


\subsubsection{NUEVO \texttt{jDUreport} o \texttt{junctionDUreport}: A Junction-centered analysis of alternative splicing. Junction-based differential signal}
...
\texttt{ASpliAS} object containing comprehensive results that support evidence of alternative usage of bins. 
...
default this number is five. (Default = 5)

For each experimental junction identified, it is also reported if it is novel ({\textit known CHEQUEAR})
and which bins it spans. In addition, it is stated if the junction is 
...
\subsubsection*{Details: PIR and PSI metrics}
\label{sec:psir}
...
used to quantify AS \cite{pmid21057496}, are calculated for bins junctions (see figure
\ref{fig:pirEq} ). 
Both, the PIR and PSI metrics, strongly enrich the 
...
\subsubsection{Estimating differential gene expression and bin/junction usage}
...
annotated genes that are considered to be expressed. For this, the average number of reads for each gene and condition and the average read density for each gene and condition are
...
Both, \texttt{DUreport} and \texttt{DUreportBinSplice} methods perform gene DE
analysis with the same methodology and give the same results. 
...
\texttt{Control} and \texttt{Treat} conditions, represented as $Treat - Control$ 
can be expressed as:
...
methods is \texttt{ [ 'Control', 'Treat' ]}. 
...
\begin{description}
  ...
  considered in the analysis. The default value is 0.05. (Default=0.05)
  ...
  \texttt{FALSE} ( see \secref{sec:du} ) (Default=FALSE)
  ...
  value is \texttt{geneMode}. (Default=geneMode)
  ...
  \texttt{geneMode} (see \secref{sec:du}). The default value is \texttt{TRUE}. (Default=TRUE)
  ...
  be ignored in the analysis. The default value is \texttt{TRUE}. (Default=TRUE)
  ...
  \item[filterWithConstrasted] a logical value that specifies that gene and   bin filtering when more than two conditions are present, should be done only with the data of those conditions and ignore the others. It is strongly recommended not to change this value.  (Default value=FALSE)
...
\begin{description}
  ...
  \secref{sec:targetsDef}.
  \item[minGenReads] minimum value for reads in any condition required for a
  gene to be considered expressed (see \secref{sec:de} ). The default value
  is 10. (Default=10)
  \item[minBinReads] minimum value for reads in any condition required for a
  bin to be included in the analysis (see \secref{sec:du} ). The default value
  is 5.(Default=5)
...
(\secref{sec:asplidu}) for details.
...
\begin{itemize}
  ...
  \texttt{genesDE}. This method returns a \texttt{data.frame} which columns are specified below.
    \begin{description}
      \item[row.names] Gene name 
      \item[symbol] An optional common name for the gene, that must be provided at
      the moment of feature extraction (see section
      \secref{sec:binDefinition}).
  ...
  columns:
    \begin{description}
      \item[row.names] Bin name. See section \ref{sectionBinG}:\nameref{sectionBinG}.
      \item[feature] Type of bin: \textbf{E} for exonic, \textbf{I}
      for intronic and \textbf{Io} for original introns. See section \ref{sectionBinG}:\nameref{sectionBinG}.
      \item[event] Alternative splicing event asigned to the bin (see section
      \ref{sec:binDefinition}:\nameref{sec:eventAssign})
      \item[locus] The name of the locus that contains the bin.
      \item[locus\_overlap] All other overlapping \textit{loci}.
      \item[symbol] An optional common name for the gene, that must be provided at
      the moment of feature extraction (see section \ref{sec:binDefinition}:\nameref{sec:binDefinition}).
      \item[gene\_coordinates] Show the genomic coordinates of the gene with
      format \texttt{chromosome:start-end}.
      ...
      \\
    \end{description}
  \item Results for experimental junctions assuming those
  exons are involved in alternative splicing events can be accessed with
  \texttt{junctionPSI} method. The returning value is a \texttt{data.frame} with
  columns:
    \begin{description}
      \item[row.names] Junction name in format \texttt{chromosome.5'site-3'site}.
...
\subsection{Reports: AspliMergedReport. Output and results}
\label{sec:output}
...
bam <- loadBAM(targets)
@
Overlap alignments with the features:
