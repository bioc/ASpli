...
When the minimum local gene model contains two isoforms (see first row of figure \ref{fig:binAssignation})
bins are labelled as follow:
...
%proposed in the \texttt{edgeR} package. 
