...
\item IMPORTANT: replicate merged \texttt{BAM} files for each contrasted condition are needed to estimate uniform coverage statistics and to generate graphical reports through the {\texttt exportIntegratedSignals} function. 
...
Note: for a large number of experimental units you might disable the generation of graphs setting the option \texttt{makeGraphs=FALSE} in \textttt{exportIntegratedSignals} function call.
