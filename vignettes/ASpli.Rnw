...
\subsubsection{Splicing reports}
...
This function integrates differential usage information using/from multiple sources. It uses bin's usage and junction's usage and merges them in three steps. 
\begin{enumerate}
\item Merges all bins with annotated junction information (jir, jes, jalt). 
...
\subsubsection{Integrate signals}
Integrates differential usage in specific regions. It returns the signals present in this specific region filtered by different parameters. \\
To prepare this table, the region is marked with a 1 in the corresponding signal if the signal in that region was originated in that particular event. If other signals match the region, the other signals are also marked as 1. If other signals overlap but don't match, then those signals are marked with 
<<jDUreport, echo=TRUE, eval=FALSE>>=
is      <- integrateSignals(sr, asd)
@
...
\subsubsection{Reports: exportSplicingReport()} \label{sec:reports}
