...
file). \texttt{Make sure all files use the same coordinate system}.
...
\begin{description}
  ...
  \item[ASpliCounts] Contains counts of aligned reads to each genomic featur
...
\subsubsection{Defining the targets}
...
\subsection{Counting reads overlapping features} 
...
\subsubsection*{Details: Counting intron flanking regions }
...
tal vez intron flanking counts va aca tab?
...
\begin{itemize}
  ...
  counted more than onece, because they might  overlap to more than one bin. This mode is not available for junctions.
\end{itemize}       


   \item {\bf diffSpliceDGE}. This alternative delegates differential bin usage estimation to \texttt{edgeR}'s 
         \texttt{ diffSpliceDGE }. This is available only for bins. This method uses an strategy
          similar to the offset methodology. ASpli allow its use with the method \texttt{DUreportBinSplice}.
\end{enumerate}
