...
\subsubsection*{E1I (\texttt{e1icounts}) and IE2 (\texttt{ie2counts}) }
...
\item The last columns are the PIR metrics calculated for each condition. The PIR metric is calculated as: PIR = (J1 + J2)/(J1 + J2 + 2*J3) (junctions are the sum by condition.)
\end{description}
...
Information about statistically significant changes in junction usage inside {\em locale}-juntion clusters.
  \begin{description}
