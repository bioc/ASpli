...
Genome-wide analysis of alternative splicing has been a very active field of research since 
the early days of NGS (Next generation sequencing) technologies.  Since then, evergrowing 
data availability and the development of increasingly sophisticated analysis methods have uncovered 
the complexity of the general splicing repertoire.  

In this vignette we describe how to use \tettt{ASpli}, an integrative and user-friendly
R package that facilitates the analysis of changes in both annotated and novel 
AS events. 

%This package combines statistical information from exon, intron, and 
...
%annotated and novel AS events. ASpli can be used to analyze both simple 
%and complex experimental designs involving multiple experimental conditions.

<<loadAspli, echo=FALSE, eval=FALSE>>=
devtools::load_all("~/Projects/RNAseq/ASpli/gitlab/ASpli")
@
...
Extract features from genome, define {\em targets} data.frame with phenotype data, and
{\em mBAMs} data.frame with phenotype data for merged BAMs:
...
\textt{ASpli} is available at Bioconductor site and can be installed following these steps:
\texttt{biocLite()}:
...
\item Optionally: replicate merged \texttt{BAM} files for each contrasted condition are needed to estimate uniform coverage statistics and to generate graphical reports through the {\texttt exportIntegratedSignals} function. 

\end{itemize}


\section{Using \texttt{ASpli}}
The workflow intended to be used with \texttt{ASpli} is divided in four main steps (Figure \ref{fig:ASpliStructure}). \begin{enumerate}
...
\begin{itemize}
  ...
  \item \texttt{ASpliAS}: Contains counts of aligned junctions   \item \texttt{ASpliDU}: Contains results of Differential expression and usage
...
When more than two isoforms are present, we still found it useful to use the three-bin local model to segment follow up analysis. For these cases (see rows 2-4 of Figure~\ref{fig:binAssignment}) ASpli identify splicing events that involve: intronic subgenic regions surrounded by exons in at least one isoform (bin labelled as IR*), exonic subgenic regions surrounded by two introns in at least one isoform (bin labelled as ES*), exonic regions surrounded by intronic and exonic neighbor bins (bin labelled as Alt5'SS* or Alt3'SS*). 
...
that has as many rows as samples. The first column  should be named \textit{bam} and should contain the path to a BAM file. Next columns should be used to specify the different experimental factors for each sample. %For a single factor experiment with 2 conditions (Control and Treatment) and 2 replicates for each condition, \textbf{targets} dataframe could be defined as follow:

%<<targetsDF, echo=TRUE, eval=TRUE>>=
%targets <- data.frame(
%  bam = c("path_to_bams/CT_1.BAM", "path_to_bams/CT_2.BAM","path_to_bams/CT_3.BAM",
%          "path_to_bams/TR_1.BAM", "path_to_bams/TR_2.BAM","path_to_bams/TR_3.BAM"),
%  genotype = c( "CT", "CT", "CT",  "TR", "TR", "TR" ),
%  stringsAsFactors = FALSE )
%@ 
%Complex designs, for example a 2 factor experiment (e.g. genotype and time), 2 values for each %factor and 2 replicates for each condition, the \textbf{targets} can be defined like this:
...
The {\em targets} data frame contains experimental factor values for each sample. \texttt{ASpli} generates simple code-name for each experimental condition concatenating the corresponding factor levels. They can be check using the \texttt{getConditions()} function. 

<<targetsDF2b, echo=TRUE, eval=FALSE>>=
getConditions( targets )
@
  
The order of the conditions given by \texttt{getConditions} is the same that in contrast argument of DUreport.norm function.
...
The method  \texttt{gbCounts()} allows to count the number of reads that overlaps each defined feature (i.e. genes, bins, junctions and intron/exon flanking regions). For genes and bins, read density values are also computed as the ratio between the number of reads and the length of a given feature. 
...
Only those reads which minimum overlap \textit{minReadLength} and without gap in this interval are considered. 
As a consequence, only reads aligned \textbf{within} those two exons/introns are counted. 
...
\item For a given gene, the read count number is computed as
...
\begin{itemize}
%\item differential usage of bins: ASpli compares log-fold-changes of bins respect to the log-fold-change of the corresponding gene.
%\item differential intron retention (junction-based): ASpli analyzes changes in junction {\em anchorage} by considering  log-fold-changes of a given experimental junction relative to changes in left and right flanking regions. 
%\item differential junction strength signal: ASpli defines junction-clusters as  sets of junctions that share at least one end with another junction of the same cluster (see Panel E of Figure \ref{fig:binsNjunctions}). In order to weigh changes for a given junction along experimental conditions, ASpli considers the log-fold-change of the analyzed junction relative to the mean log-fold-change of junctions of the same cluster. 
\item bin-coverage signal:  ASpli assesses for differential usage of bins comparing bin's log-fold-changes  with the overall log-fold-change of the corresponding gene.
\item junction anchorage signal: For every experimentally detected junction, ASpli analyzes differential intron retention changes by considering  log-fold-changes of a given experimental junction relative to changes in coverage of left and right junction flanking regions. 
\item junction locale signal: In the same spirit than MAJIQ and LeafCutter, ASpli defines junction-clusters as  sets of junctions that share at least one end with another junction of the same cluster (see Figure \ref{fig:jcluster}). In order to characterize changes for a given junction along experimental conditions, ASpli weighs log-fold-change of the junction of interest relative to the mean log-fold-change of junctions belonging to the same cluster. 

\end{itemize}

ASpli makes use of the functionality implemented in the diffSpliceDGE function of the edgeR package to perform all of this comparisons within a unified statistical framework. Given a set of elements (i.e. bins or junctions) of a certain group (i.e. genes, anchorage group or junction-cluster), a negative binomial generalized log-linear model is fit at the element level, considering an offset term that accounts for library normalization and collective changes. Differential usage is assessed testing coefficients of the GLM. 
At the single element-level, the relative log-fold-change is reported along with the associated p-value and FDR adjusted q-values. In addition a group-level test is considered to check for differences in the usage of any element of the group between experimental conditions (see {\em diffSpliceDGE} documentation included in edgeR package for details \cite{Robinson2010}).
...
Export tab delimited table (see \secref{sec:reports} for interactive html reports):
...
\item otherSources: If user wants to compare ASpli results with results from other methods, {\em otherSources} must be a GenomicRange object with all the regions found with the other methods. It will be integrated with a new column next to signals information.
...
\item openInBrowser: logical. Whether results are automatically displayed just after exporting.	
\item maxBinFDR (Default = 0.2) Only export info for bins with FDR < maxBinFDR
\item maxJunctionFDR (Default = 0.2): Only export info for junctions with FDR < maxJunctionFDR
\end{itemize}
...
\begin{itemize}
 \item a genome with ten genes with multiple isoforms
 ...
 \item a \textbf{GTF} file with the corresponding annotation. 
 \end{itemize}
...
library( GenomicFeatures )
@
Load a small {\em gtf}. In this case \texttt{gtfFileName} contains the full path to the example gtf file in your system.


<<Ex02.b, echo=TRUE, eval=TRUE>>=
gtfFileName <- aspliExampleGTF()
@

Then, create a TxDb object using \texttt{makeTxDbFromGFF}   
<<Ex02.c, echo=TRUE, eval=TRUE>>=
genomeTxDb <- makeTxDbFromGFF( gtfFileName )
@

Extract the genomic features (\texttt{binGenome} should produce a brief summary of the parsed genome).
<<Ex02.d, echo=TRUE, eval=TRUE>>=
...
@
Define the {\em targets} data.frame:
...
Experimental conditions are inferred from the experimental factors appearing in the {\em target} data.frame:
<<Ex02.g, echo=TRUE, eval=FALSE>>=
getConditions(targets)
#[1] "A_C" "A_D" "B_C" "B_D"
@

We define a data.frame with path information for merged bams \footnote{BAM files obtained combining replicates of the same experimental conditions (estimated using 'samtools merge ...'). These files are needed to estimate non-uniformity coverage and to produce graphical output reports}.
...
Table \ref{tab:Contrasts01} shows different examples of contrasts that can be used to test for different hypothesis:
...
test whether the differential patterns observed between D and C levels of \tetbf{f2} are different for 
A and B \textbf{f1} levels. The interaction contrast corresponds to:
\begin{equation}
I = ( B.D - B.C ) - ( A.D - A.C ) 
\end{equation}

As Table \ref{tab:Contrasts01} shows, the coefficients of the terms in this expression can be 
represented in the {\em contrast} vector $[ 1,-1,-1, 1]$, where we have taken the order given
by \texttt{getConditions} function in consideration.
...
\texttt{genesDE()} and \texttt{binsDU()} accesors report the statistical significance results. We can see that in this example no evidence of a statistically significant interaction term was found at gene expression level. However, 9 out of 14 bins displayed statistically significant interaction effect (fdr < 0.01) 
<<Ex02.k, echo=TRUE, eval=TRUE>>=
genesDE(gb)
binsDU(gb)
@
...
<<Ex02.l, echo=FALSE, eval=TRUE>>=
jdur    <- jDUreport(asd, 
                     contrast =  c( 1, -1, -1, 1 ) ,
                     runUniformityTest = TRUE,
                     mergedBams = mBAMs)
@
<<Ex02.la, echo=TRUE, eval=FALSE>>=
jdur    <- jDUreport(asd, 
                     contrast =  c( 1, -1, -1, 1 ) ,
                     runUniformityTest = TRUE,
                     mergedBams = mBAMs)
...
Results for junction-based events matching bin ranges can be accessed through \texttt{jir(jdur)}, \texttt{jes(jdur)} and \texttt{jalt(jdur)} accesors (see \secref{sec:jdureport}).
...
<<Ex02.kk, echo=TRUE, eval=TRUE>>=
localec(jdur)
@
Rows are ordered by FDR values. Row names correspond to cluster-ids. The {\em participation} and {\em dParticipation} values for the cluster junction displaying the most significant changeare shown in the last two columns of the table. Note that NA's are reported for cluster-7 as no statistically significant changes were found for any junction of that cluster. 
...
This formula correspond to the following model matrix:
<<Ex05, echo=TRUE, eval=TRUE>>=
 model.matrix(form,targets)
@
Table \ref{tab:Formulas01} summarizes three statistical comparisons that could be implemented considering the second, third and fourth coefficient of the model:
\begin{table}[H]
  \noindent\makebox[\textwidth]{%
    \begin{ccc}
...
The {\em formula} approach can easily handle paired designs.
...
targets <- targets[c(1,4,7,10),]
formula <- formula(~f1+f2)
...
\section{Appendix: Detailed Ouputs}
Here is the content of all the info present in \texttt{ASpli} objects. Some of them are common in several tables.
\label{sec:outputs}
ASpliCounts:
\begin{itemize}
  ...
  \item Bin counts (\texttt{countsb}) and bin read densities (\texttt{rdsb}) dataframes contain the following information:
    \begin{description}
      \item[row.names]: see \secref{sectionBinG}.
      \item[feature] Options are:  \textbf{E} for exonic bins, \textbf{I}
  ...
  \item Junction counts (\texttt{countsj}) dataframe contains the following information:
    \begin{description}
  ...
  \item E1I (\texttt{e1icounts}) and IE2 (\texttt{ie2counts}) flanking region count dataframes contain the
...
\begin{itemize}
 \item{localec}: data.frame with information about statistically significant changes in junction usage inside {\em locale}-juntion clusters.
  \begin{itemize}
   \item size: number of junctions belonging to the cluster.
   \item cluster.LR: likelihood ratio of cluster differential usage.
   \item pvalue: pvalue of cluster differential usage.
   \item FDR: fdr of cluster differential usage.
   \item range: cluster location.
   \item participation: participation of the significant junction (FDR < maxFDRForParticipation) presenting maximal participation value inside the cluster
 ...
 \item{localej}
  \begin{itemize} 
   \item cluster: name of the cluster the junction belongs to
   ...
   \item pvalue: pvalue of junction
   \item FDR: FDR of junction
   \item annotated: is junction annotated or new
   \item participation: the maximal participation value observed across contrasted condictions
   \item dParticipation: delta participation of the maximal participation value observed across contrasted condictions
   \item From dParticipation to the end, junction counts for all samples
  \end{itemize}  
 \item{anchorc}
  \begin{itemize}
   \item cluster.LR: likelihood ratio of cluster differential usage.
 ...
 \item{anchorj}
  \begin{itemize}
   ...
   \item pvalue: pvalue of junction
   \item FDR: FDR of junction
   \item J1.pvalue: pvalue of J1 junction
   \item J2.pvalue: pvalue of J2 junction
   ...
   \item annotated: is junction annotated or new
   \item From annotated to the end, junction counts for all samples
  \end{itemize}   
 \item{jir}
  \begin{itemize}
   \item J3: J3 junction/s
   \item logFC: log fold change of junction accross conditions
   \item log.mean: log of mean counts accross all conditions for this junction
   \item pvalue: pvalue of junction
   \item FDR: FDR of junction
   ...
   \item From multiplicity to the end, junction counts for all samples
  \end{itemize}      
 \item{jes}
  \begin{itemize}
   \item event: type of event
   \item J3: J3 junction/s
   \item logFC: log fold change of junction accross conditions
   \item log.mean: log of mean counts accross all conditions for this junction
   \item pvalue: pvalue of junction
