...
setMethod(
  ...
  definition = function( is, output.dir="is", sr, counts, features, mergedBams, jCompletelyIncluded = FALSE, zoomRegion = 1.5, useLog = FALSE, tcex = 1, ntop = NULL) {
    ...
    if(!all(colnames(is) %in% c('region','locus','b','bjs','ja','jl','bin','feature','bin.event','J3','binreg','locus_overlap','b.fdr','b.logfc','bjs.fdr','bjs.logfc','bjs.nonuniformity','bjs.inclussion','a.fdr','a.logfc','a.nonuniformity','a.participation','l.fdr','l.logfc','l.participation'))){
    ...
    sketch = htmltools::withTags(table(
      ...
      thead(
        tr(
          ...
          th(colspan = 4, 'Anchor Junctions', bgcolor="#DCDCDC"),
          th(colspan = 3, 'Locale Junctions', bgcolor="#C0C0C0")
        ),
        tr(
          lapply(c("logFC", "FDR", "logFC", "FDR", "Non Uniformity", "Inclussion", "logFC", "FDR", "Non uniformity", "Participation", "logFC", "FDR", "Participation"), th)
        )
        
      )
    ))
    y <- datatable(cbind('&oplus;', is[1:ntop,c("region", "locus", "locus_overlap", "b", "bjs", "ja", "jl", "bin", "feature", "bin.event", "b.logfc", "b.fdr", "bjs.logfc", "bjs.fdr", "bjs.nonuniformity", "bjs.inclussion", "a.logfc", "a.fdr", "a.nonuniformity", "a.participation", "l.logfc", "l.fdr", "l.participation")]),
              rownames = FALSE,
              escape = -1,
              filter ="top",
              fillContainer = F,
