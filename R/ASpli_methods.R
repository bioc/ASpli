...
setGeneric( name = "exportIntegratedSignals", 
            def = function ( is, output.dir="is", 
                             sr, counts, features, 
                             exons, mergedBams, genePlot = TRUE, 
                             jCompletelyIncluded = FALSE, zoomRegion = 1.5, 
                             chrMap = NULL, useLog = FALSE, tcex = 1
                             ) standardGeneric( "exportIntegratedSignals" ) )

setMethod(
  f = "exportIntegratedSignals",
  signature = "data.table",
  definition = function( is, output.dir="is", sr, counts, features, exons, mergedBams, genePlot = TRUE, jCompletelyIncluded = FALSE, zoomRegion = 1.5, chrMap = NULL, useLog = FALSE, tcex = 1) {
    
    if(!all(colnames(is) %in% c("region", "locus", "b", "bjs", "ja", "jl", "bin", "feature", "bin.event", "J3", "binreg"))){
    ...
    for(i in 1:length(is$region)){
      r <- is$region[i]
      if(i %% floor(length(is$region)/10) == 0){
        message(paste0(i/floor(length(is$region)/10)*10, " % completed"))
      }
      tryCatch({
        png(width = 1400, height=700, filename = paste0(getwd(), "/", output.dir, "/img/", r, ".png"))
        .plotSplicingPattern(r, is, counts, features, mergedBams, sr, exons, genePlot, jCompletelyIncluded, zoomRegion, chrMap, useLog, tcex)
        dev.off()
    ...
    y <- datatable(cbind(' ' = '&oplus;',is[,1:8]),
              escape = -2,
              filter ="top",
              extensions = 'Buttons',
              options = list(dom = 'lfrtBip',
