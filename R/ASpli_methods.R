...
setMethod(
  ...
  definition = function( is, output.dir="is", sr, counts, features, exons, mergedBams, jCompletelyIncluded = FALSE, zoomRegion = 1.5, chrMap = NULL, useLog = FALSE, tcex = 1) {
    ...
    for(i in 1:5){
      ...
      tryCatch({
        ...
        .plotSplicingPattern(r, is, counts, features, mergedBams, sr, exons, genePlot = TRUE, jCompletelyIncluded, zoomRegion, chrMap, useLog, tcex)
        dev.off()
        png(width = 1400, height=700, filename = paste0(getwd(), "/", output.dir, "/img/", r, ".png"))
        .plotSplicingPattern(r, is, counts, features, mergedBams, sr, exons, genePlot = FALSE, jCompletelyIncluded, zoomRegion, chrMap, useLog, tcex)
        dev.off()
    ...
    y <- datatable(cbind(' ' = '&oplus;',is[,c("region", "locus", "b", "bjs", "ja", "jl", "bin", "feature", "bpval", "lpval", "apval")]),
              escape = -2,
              filter ="top",
              extensions = 'Buttons',
              options = list(dom = 'lfrtBip',
