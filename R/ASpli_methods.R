...
setMethod(
  ...
  definition = function( ir, output.dir="ir" ) {
    ...
    bamFiles <- c("/xdata1/porcupine_data/STAR/mergedBAMS/col_16.star.bam","/xdata1/porcupine_data/STAR/mergedBAMS/pcp_16.star.bam")
    mergedBAMs <- data.frame(bam=bamFiles,condition=c("col_16","pcp_16"))
    chrMap <- as.character(1:5)
    names(chrMap) <- paste0("Chr",1:5)
    #.plotSplicingPattern("1.10054761.10054865", genomeTxDb = 
    ...
    y <- datatable(cbind(' ' = '&oplus;',ir[,1:8]),
            ...
            var format = function(d) {
              alert(d);
             return '<img src=\"sashimi_sample.png\" height=\"350\"></img>';
            };
