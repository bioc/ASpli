...
setGeneric( name = "exportIntegratedSignals", 
            def = function ( ir, output.dir="ir"  ) standardGeneric( "exportIntegratedSignals" ) )

setMethod(
  f = "exportIntegratedSignals",
  signature = "data.table",
  definition = function( ir, output.dir="ir" ) {
    
    file.exists( output.dir ) || dir.create( output.dir )
    file.exists( paste0(output.dir, "/img") ) || dir.create( paste0(output.dir, "/img") )
    if(!all(colnames(ir) %in% c("region", "locus", "b", "bjs", "ja", "jl", "bin", "feature", "bin.event", "J3", "binreg"))){
      stop("ir must be a data.table generated by integrateSignals method")
    }
    ir[,b:=as.factor(b)]
    ir[,bjs:=as.factor(bjs)]
    ir[,ja:=as.factor(ja)]
    ir[,jl:=as.factor(jl)]
    ir[,feature:=as.factor(feature)]
    ir[,bin.event:=as.factor(bin.event)]
    
    
    #Necesarios para correr
    (load("~/Downloads/features.RData"))
    bamFiles <- c("/xdata1/porcupine_data/STAR/mergedBAMS/col0_16.star.bam","/xdata1/porcupine_data/STAR/mergedBAMS/pcp_16.star.bam")
    mergedBAMs <- data.frame(bam=bamFiles,condition=c("col_16","pcp_16"))
    chrMap <- as.character(1:5)
    names(chrMap) <- paste0("Chr",1:5)
    
    for(i in ir$region[1:10]){
      png(width = 1000, height=500, filename = paste0(getwd(), "/", output.dir, "/img/", i, ".png"))
      .plotSplicingPattern(i, iss, counts, f, mergedBAMs, sr, exones, chrMap = chrMap)
      dev.off()
    }
    
    y <- datatable(cbind(' ' = '&oplus;',ir[,1:8]),
              escape = -2,
              filter ="top",
              extensions = 'Buttons',
            ...
            var format = function(d) {
             return '<img src=\"img/' + d[2] + '.png\" height=\"350\"></img>';
            };
